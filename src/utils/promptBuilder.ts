...
export function buildPromptPreview(config: PromptConfig, sample?: Project | null): string {
  ...
  return [
    ...
    '',
    'Основные условия проекта КС заполни таблицей: предмет, объект, сроки, инвестиции, участие концедента (грант / недополученные доходы / проезд нарушителей), концессионная плата, проектирование, подготовка территории, ЗУ, обеспечение, компенсация при прекращении, особые обстоятельства, ответственность, прямое соглашение.',
    ...
    'В описании оцени: соответствие императивным нормам закона; реалистичность исполнения при отсутствии ПД и ЗУ; финансовую целесообразность для инвестора.',
    '',
