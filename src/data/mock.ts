...
export const PROMPT_SECTIONS: { id: PromptSectionId; title: string; hint: string }[] = [
  ...
  { id: 'description', title: 'Описание проекта', hint: 'что строится / модернизируется, сроки, инициатор' },
...
export const DEFAULT_PROMPT: PromptConfig = {
  ...
  extraInstructions:
    'Если в файле нет ставки дисконтирования — явно пометь как допущение и используй отраслевой ориентир. Пояснения сотрудника имеют приоритет над догадками модели.',
}
